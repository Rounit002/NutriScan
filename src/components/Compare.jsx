      ...
      <div className="compare-shell">
        ...
        {visibleIsLoading ? (
          ...
          <div className="compare-empty">
            ...
            <h2>Need More Scans</h2>
            <p>Scan at least 2 products to start comparing health scores.</p>
          </div>
        ) : (
              ...
              <div className="compare-picker">
                ...
                <div className="compare-product-list">
                  {paginatedScans.length > 0 ? paginatedScans.map((scan) => {
                    ...
                    const scoreColor = scan.score >= 8 ? '#10B981' : scan.score >= 5 ? '#fd761a' : '#ba1a1a';
                ...
                {selectedScans.length > 0 && (
                  <div className="compare-action-bar animate-streak-pop">
                    ...
                    >
                      <BarChart2 size={24} />
                      {selectedScans.length < 2 ? `Select ${2 - selectedScans.length} more` : `Compare ${selectedScans.length} Choices`}
                    </button>
                  </div>
                )}
              </div>
                      ...
                      <div key={`compare-${scan.id}`} className="min-w-[280px] w-[280px] ns-card !p-6 flex flex-col gap-6 relative overflow-hidden snap-center group border-2 border-transparent">
                        ...
                        <div className="text-center space-y-4">
                          ...
                          <div className="relative inline-block">
                            ...
                            <div className="w-20 h-20 rounded-[24px] border-2 flex flex-col items-center justify-center relative z-10 shadow-sm"
                              ...
                              <span className="text-[8px] font-black uppercase tracking-widest mt-1 opacity-80" style={{ color: scoreColor }}>Health Score</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 space-y-3">
                          <div className="flex items-center gap-2 px-1">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--ns-on-surface-var)' }}>Key Insights</span>
                          </div>
                          ...
                          <div className="flex flex-col gap-2">
                            {verdictItems.length > 0 ? verdictItems.map((point, pIdx) => {
                              ...
                              const dotColor = isGood ? '#10B981' : isBad ? '#ba1a1a' : (scan.score >= 8 ? '#10B981' : scan.score >= 5 ? '#fd761a' : '#ba1a1a');
                              const textColor = 'var(--ns-on-surface)';
                              const bgColor = isGood ? 'rgba(16,185,129,0.08)' : isBad ? 'rgba(186,26,26,0.06)' : 'var(--ns-surface-low)';

                              return (
                                <div
                                  key={pIdx}
                                  className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl border"
                                  style={{ background: bgColor, borderColor: isGood ? 'rgba(16,185,129,0.1)' : isBad ? 'rgba(186,26,26,0.1)' : 'var(--ns-outline-var)' }}
                                >
                                  {isGood ? (
                              ...
                              <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl" style={{ background: 'var(--ns-surface-low)', border: '1px solid var(--ns-outline-var)' }}>
                                ...
                                <span className="text-[11px] font-bold leading-snug" style={{ color: 'var(--ns-outline)' }}>No data available</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="pt-4 border-t flex items-center justify-between" style={{ borderColor: 'var(--ns-outline-var)' }}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--ns-outline)' }}>Nutri Scan</span>
                          </div>
                          {scan.score >= 8 ? (
                            ...
                            <div className="flex items-center gap-1.5" style={{ color: '#ba1a1a' }}>
                              <XCircle size={14} />
                              <span className="text-[10px] font-black uppercase tracking-widest">Risky</span>
                            </div>
                          ) : null}
                        </div>
                      </div>
